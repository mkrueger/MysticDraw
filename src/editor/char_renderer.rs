...
impl CharRenderer {
            ...
            for color in 0..16 {
                for ch in 0..=255 {
                    ...
                    for y in 0..16 {
                        ...
                        for x in 0..8 {
                            if (line & (128 >> x)) != 0 {
                                texture.push(crate::buffer::DOS_DEFAULT_PALETTE[color].0);
                                texture.push(crate::buffer::DOS_DEFAULT_PALETTE[color].1);
                                texture.push(crate::buffer::DOS_DEFAULT_PALETTE[color].2);
                            } else {
                                texture.push(0_u8);
                                texture.push(0_u8);
                                texture.push(0_u8);
    ...
    pub fn draw(&self) {
        ...
        unsafe {
            let buffer = &crate::buffer::ALL_BUFFERS[self.buffer_id.get()] ;
            for y in 0..buffer.base_layer.height {
                for x in 0..buffer.base_layer.width {
                    let mut ch  = buffer.get_char(&Position::from(x, y));
/* 
